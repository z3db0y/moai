	...
	switch (cmd) {
		...
		case 'connect':
			if (args[0]) {
				try {
					new URL(args[0]);
				} catch (err) { return console.log('Invalid URL Provided.'); }
				client.connect(new URL(args[0]), (s) => s ? console.log('Connected to', args[0]) : console.log('Failed to connect to', args[0]));
		...
		case 'disconnect':
			try {
				client.disconnect();
				console.log('Disconnected from', client.socket.url);
			} catch(err) { console.log(err.message); }
			break;
		case 'exit':
			process.exit(0);
